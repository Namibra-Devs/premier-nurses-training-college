  ...
  return (
    <div
      className={`bg-white h-full my-auto overflow-hidden pr-2 md:pr-4 flex flex-col items-start justify-start gap-2 transition-all duration-700 ease-in-out ${
        open ? "w-[700px] md:w-[350px]" : "w-[40px]"
      }`}>
      {tabs.map((tab, index) => (
        <button
          key={index}
          className={`flex items-center w-full text-left gap-2 pl-0.5 py-2 text-sm leading-[10px] ${
            currentTab === index
          ...
          <span className={`${open ? "opacity-100" : "opacity-0"} transition-opacity duration-200`}>{tab.label}</span>
        </button>
      ))}
    </div>
  );
